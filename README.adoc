...
The `cap/` directory is the Capsium package source:

----
cap/
├── metadata.json   # Package metadata (name, version, guid, uuid, license, ...)
└── content/        # Servable content root
    ├── index.html  # Root file (entry point)
    ├── script.js   # TOC, scroll progress, Markdown rendering glue
    ├── styles.css  # Story styling
    ├── vendor/
    │   └── marked.min.js  # Vendored marked.js (MIT) — the package is fully self-contained
    └── images/     # Character illustrations
----

`manifest.json`, `routes.json`, `storage.json` and `security.json` are
generated automatically when the package is packed or loaded; `security.json`
contains the SHA-256 checksums reactors use to verify package integrity.
...
# or
$ bundle exec capsium reactor serve https://github.com/capsiums/cap-story/releases/download/v1.0.0/story-of-claire-1.0.0.cap
----

Replace `https://github.com/capsiums/cap-story/releases/download/v1.0.0/story-of-claire-1.0.0.cap` with the actual URL of your `.cap` file hosted on GitHub or another platform.
...
Package created: story-of-claire-1.0.0.cap
----
...
[source,sh]
----
$ bundle exec capsium package info cap       # overview: metadata, routes, storage
$ bundle exec capsium package manifest cap   # generated manifest.json
$ bundle exec capsium package routes cap     # generated routes.json
$ bundle exec capsium package validate cap   # structural + integrity checks
----
